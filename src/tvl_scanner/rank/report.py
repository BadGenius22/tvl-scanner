...
def _bounty_summary_header(candidates: list[CandidateRecord], scan_date: date) -> str:
...
def write_report(
    ...
    *,
    ...
    is_bounty = bool(candidates) and all(c.priority_formula == "bounty" for c in candidates)
    if is_bounty:
        body = (
            _bounty_summary_header(candidates, scan_date)
