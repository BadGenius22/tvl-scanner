...
| 12 payout quality | `--require-vault`, `--exclude-pay-to-submit`, `--exclude-premium` |
...
- **`--exclude-premium` is a mapping, not a field.** Immunefi publishes no "premium" concept; the closest real one is the `Subscription Plan: Essential/Pro/Elite` feature, which is what the *project* buys from Immunefi rather than a researcher-facing gate. The fee that actually lands on a researcher is `Pay to Submit` (28 programs), handled by its own `--exclude-pay-to-submit`. 20 programs carry both, so the flags are kept independent. `pay_to_submit` is scored on BOTH criterion 11 (+1.0 — a per-report fee thins the field) and criterion 12 (−2.0 — it is a cost shifted onto you); those effects are real and opposite, so each is scored where it belongs instead of netted into one number. The subscription tier is deliberately unscored.
