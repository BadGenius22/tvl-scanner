...
#### Reading the program as a document, not just a score

Alongside the rubric, the profile carries what decides whether a finding is *submittable at all*, which is upstream of how highly it would score. Four fixes, each motivated by a measured defect in the live catalogue:

- **`scope_assets` is the full assets-in-scope table**, not a count and one representative address. The standing rule is to extract and verify the whole in-scope list before starting; a count sends you back to the program page to do it by hand. It lifts into the frontmatter as `bounty_scope_assets`, and `repo` / `address` are separate fields because repo- and path-level scope is a different audit shape (source on a branch, matched back to deployed bytecode by hand) from a deployed address.
- **Primacy-of-Impact placeholders are excluded from every count.** Immunefi's sentinel row points at `immunefi.com` and exists only to carry `isPrimacyOfImpact: true`; 80 of them sit across 61 of 247 programs. Counting them as contracts made Twyne read as 16 in-scope contracts against 15 real addresses, in the same record that printed 15 elsewhere. Removing them changes criterion 8's score on 21 programs. The rows are kept in `scope_assets` and flagged, never dropped, so the record and the program page agree line for line.
- **Placeholder audit rows no longer set recency.** Programs routinely fill `audits[]` with one link to their own security page under a category label ("All Audits", "Audit") dated to the day the bounty launched — 16 of 237 rows. Scored as a review, that made `days_since_latest_audit` equal `program_age_days` exactly, and criterion 7 carries the largest weight (0.15). `verified_audit_count` counts real rows; `latest_audit_at` comes from those only, so a placeholder-only program reads UNDATED instead of freshly audited. `audit_count` still reports every listed row, so Stage 3's audit-density folding is unaffected.
- **`pocPerTypeAndSeverity` is parsed.** The live catalogue never sets `pocRequired` on a reward row and states the fact once program-wide instead, so every tier of every record came out `null`. 151 programs now resolve a definite answer for critical.
